...
The tooling for that evaluation exists and is documented in [LIVE_EVALUATION.md](./LIVE_EVALUATION.md): a budget-guarded single-call canary and a deterministic harness that measures contract pass rate, validation failure layers, stance adherence, judge-text diversity, cost, tokens, and latency across a fixed fixture corpus. Judge decisions remain individual call records and are never counted into a panel-level result. Both tools default to dry-run. Neither has been run against a live provider, so no claim about any model's quality is supported yet.
